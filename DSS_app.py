...
# =====================================================
# ROUTING: Show landing page or brand deep-dive page
# =====================================================
if "selected_brand" not in st.session_state:
    st.session_state["selected_brand"] = None

if st.session_state["selected_brand"] is None:
    # --- HOME PAGE: render original landing page ---
    st.components.v1.html(html_content, height=920, scrolling=False)

    # Brand selection buttons (visible below the landing page)
    st.markdown("""<style>
    .brand-nav-row .stButton > button {
    ...
    .brand-nav-row .stButton > button:hover {
    ...
    st.markdown('<div class="brand-nav-row">', unsafe_allow_html=True)
    st.markdown("**Select a brand for Deep Dive \u2193**", unsafe_allow_html=False)
    brand_list = ["Nurtec", "Eliquis", "Prevnar", "Comirnaty", "Abrysvo", "Paxlovid", "Zavzpret", "Beyfortus"]
    cols = st.columns(8)
    for i, brand in enumerate(brand_list):
        with cols[i]:
            if st.button(brand, key=f"brand_btn_{brand.lower()}"):
                st.session_state["selected_brand"] = brand.lower()
                st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)

else:
    # --- BRAND PAGE: render brand deep-dive ---
    ...
    render_brand_page(st.session_state["selected_brand"], BRAND_CONFIG)
